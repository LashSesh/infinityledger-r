...
fn benchmark_spiral_snapshot_creation(c: &mut Criterion) {
    ...
    group.bench_function("create_snapshot", |b| {
        b.iter(|| {
            spiral.create_snapshot(
                black_box(&test_data),
                black_box("BENCH_SEED_001"),
                None,
            ).unwrap()
        })
    });
...
fn benchmark_ledger_operations(c: &mut Criterion) {
    ...
    group.bench_function("verify_chain_integrity", |b| {
        b.iter(|| {
            ledger.verify_chain_integrity(black_box(5)).unwrap()
        })
    });
...
fn benchmark_json_serialization(c: &mut Criterion) {
    ...
    group.bench_function("deserialize", |b| {
        b.iter(|| {
            serde_json::from_str::<serde_json::Value>(black_box(&serialized)).unwrap()
        })
    });
