...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/collections/:name/provider", patch(update_collection_provider))
...
async fn search(
    ...
    let search_results: Vec<SearchResult> = results.into_iter().map(|r| {
        SearchResult {
            ...
            metadata: r.get("metadata").and_then(|v| v.as_object()).map(|obj| {
...
async fn list_collections(State(state): State<AppState>) -> Result<Json<CollectionsResponse>> {
    ...
    for (name, coll_state) in index_manager.collections.iter() {
        ...
        let dimensions = coll_state.vectors.values()
            .next()
        ...
        collections.push(CollectionInfo {
            ...
            dimensions,
            provider: coll_state.indexes.get("provider")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
        });
    }
...
async fn list_collection_vectors(
    ...
    let coll_state = index_manager.collections.get(&collection)
    ...
    let vectors: Vec<VectorPayload> = coll_state.vectors.iter()
        .skip(offset)
        .take(limit)
        .map(|(id, vec_data)| {
            let vector = vec_data.get("vector")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|x| x.as_f64()).collect())
                .unwrap_or_else(Vec::new);
            
            let metadata = vec_data.get("metadata")
...
async fn update_collection_provider(
    ...
    index_manager.set_collection_provider(&collection, &request.provider)
...
async fn bulk_upsert_points(
    ...
    let records: Vec<VectorRecord> = request.points.into_iter().map(|v| {
        VectorRecord::new(
            v.id,
            v.vector,
            v.metadata.map(|m| m.into_iter().collect()).unwrap_or_default(),
    ...
    index_manager.upsert_vectors(&request.collection, records, None, None)
        .map_err(|e| ApiError::VectorDB(format!("Failed to upsert vectors: {}", e)))?;
    
    Ok((StatusCode::ACCEPTED, Json(BulkPointsResponse {
        job_id,
...
) -> Result<Json<BulkJobStatusResponse>> {
