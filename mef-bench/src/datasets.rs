...
pub fn iter_records<'a>(ids: &'a [String], vectors: &'a [Vec<f64>]) -> impl Iterator<Item = Record> + 'a {
    ids.iter()
        .zip(vectors.iter())
        .enumerate()
        .map(|(index, (id, vector))| {
...
pub fn generate_query_vectors(
    ...
    for _ in 0..count {
        ...
        // Add Gaussian jitter
        let jittered: Vec<f64> = base.iter()
            .map(|&value| {
...
pub fn brute_force_top_k(
    ...
    let scorer: fn(&[f64], &[f64]) -> f64 = if metric.to_lowercase() == "l2" || metric.to_lowercase() == "l2sq" {
        negative_l2_squared
    } else {
        cosine_similarity
    };
    
    let mut scored: Vec<(usize, f64)> = corpus.iter()
        .enumerate()
...
mod tests {
    ...
    fn test_iter_records() {
        ...
        assert_eq!(records[0].metadata.get("source").unwrap(), &serde_json::json!("bench"));
        assert_eq!(records[0].metadata.get("index").unwrap(), &serde_json::json!(0));
    }

    #[test]
    fn test_chunked() {
        let ids = vec!["id1".to_string(), "id2".to_string(), "id3".to_string(), "id4".to_string(), "id5".to_string()];
        let vectors = vec![
    ...
    fn test_chunked_exact_fit() {
        let ids = vec!["id1".to_string(), "id2".to_string(), "id3".to_string(), "id4".to_string()];
