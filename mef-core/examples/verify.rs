...
use mef_core::{
    MEFCore, MEFCoreConfig,
    canonical_nodes, canonical_edges, complete_canonical_edges,
    FieldVector,
};
...
fn main() -> anyhow::Result<()> {
    ...
    println!("   Scaled by 2: {:?}", scaled.as_vec().iter().map(|x| format!("{:.2}", x)).collect::<Vec<_>>());
    println!();
    ...
    println!("     r: {}, a: {}, b: {}, c: {}, k: {}", 
        config.spiral.r, config.spiral.a, config.spiral.b, 
        config.spiral.c, config.spiral.k);
    
    println!("   Solve-Coagula config:");
    println!("     lambda: {}, eps: {}, max_iter: {}", 
        config.solvecoagula.lambda, config.solvecoagula.eps, 
        config.solvecoagula.max_iter);
    
    println!("   Gate config:");
    println!("     por_delta: {}, phi_star: {}, mci_min: {}", 
    ...
    println!("{}", custom_json.lines().map(|l| format!("   {}", l)).collect::<Vec<_>>().join("\n"));
    println!();
