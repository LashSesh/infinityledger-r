...
use mef_core::{
    MandorlaField, GabrielCell, couple_cells, neighbor_feedback,
    QLogicEngine, ResonanceTensorField,
};
...
fn main() {
    ...
    println!("✓ Created MandorlaField (threshold: {}, α: {}, β: {})",
    ...
    for (i, cell) in cells.iter_mut().enumerate() {
        ...
        println!("  Cell {} activated: psi={:.3}, output={:.3}",
    ...
    println!("  Cell 1 after feedback: psi={:.3}, rho={:.3}, omega={:.3}",
    ...
    println!("✓ Created QLogicEngine with {} nodes", engine.osc_core.num_nodes);
    
    // Run multiple time steps
    println!("\n  Time evolution:");
    for t in [0.0, PI / 4.0, PI / 2.0, 3.0 * PI / 4.0, PI] {
        let result = engine.step(t);
        println!("  t={:.4}π: entropy={:.4}, centroid={:.2}, sparsity={:.4}",
                 t / PI,
                 result.entropy,
    ...
    println!("\n  Field at t=0.5:");
    println!("    Field values: [{:.3}, {:.3}, {:.3}, ...]",
             result.field[0], result.field[1], result.field[2]);
    println!("    Spectrum magnitudes: [{:.3}, {:.3}, {:.3}, ...]",
    ...
    println!("✓ Created {}x{}x{} ResonanceTensorField",
    ...
    for step in 1..=5 {
        ...
        println!("  Step {}: t={:.2}, state(0,0,0)={:.4}, coherence={:.4}, grad={:.6}, sing={}",
                 step, tensor.time, state[[0, 0, 0]], coherence, grad_norm, singularity);
    }
    ...
    println!("  State after modulation at (2,2,2): {:.4}", state_after[[2, 2, 2]]);
    ...
    println!("  Amplitude at (0,0,0): {:.1}", tensor.get_amplitude()[[0, 0, 0]]);
    println!("  Frequency at (1,1,1): {:.1}", tensor.get_frequency()[[1, 1, 1]]);
    ...
    println!("  Added {} QLogic patterns", integrated_mandorla.inputs.len());
