...
pub fn hexagon_rotation(k: i32) -> Vec<usize> {
    let k = ((k % 6) + 6) % 6; // Handle negative k
...
pub fn hexagon_reflection(axis_node: usize) -> Vec<usize> {
    ...
    for node in 2..=7 {
        ...
        let new_ang_round = if new_ang_round < 0 { new_ang_round + 360 } else { new_ang_round };
...
pub fn generate_c6_subgroup() -> Vec<Vec<usize>> {
    (0..6).map(|k| hexagon_rotation(k)).collect()
}
...
pub fn generate_d6_subgroup() -> Vec<Vec<usize>> {
    ...
    let reflections: Vec<Vec<usize>> = (2..=7).map(|i| hexagon_reflection(i)).collect();
