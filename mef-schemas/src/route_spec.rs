...
impl RouteSpec {
    ...
    pub fn validate(&self) -> crate::Result<()> {
        ...
        if self.permutation.len() != 7 {
            return Err(crate::SchemaError::InvalidRoute(
                format!("Permutation must have exactly 7 elements, got {}", self.permutation.len())
            ));
        }
        ...
        if let Some(ref sigma) = self.sigma {
            if sigma.len() != 7 {
                return Err(crate::SchemaError::InvalidRoute(
                    format!("Sigma must have exactly 7 elements, got {}", sigma.len())
                ));
            }
            // Validate range [1..8)
            for &val in sigma {
                if val < 1 || val > 7 {
                    return Err(crate::SchemaError::InvalidRoute(
                        format!("Sigma values must be in [1..8), got {}", val)
                    ));
                }
            }
        }
        ...
        if let Some(ref slots) = self.permutation_slots {
            if slots.len() != 7 {
                return Err(crate::SchemaError::InvalidRoute(
                    format!("Permutation slots must have exactly 7 elements, got {}", slots.len())
                ));
            }
        }
