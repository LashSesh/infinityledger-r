...
use crate::providers::{IndexProvider, get_provider, get_providers};
...
impl VectorRecord {
    ...
    pub fn new(id: String, values: Vec<f64>, metadata: HashMap<String, Value>, epoch: Option<i64>) -> Self {
        Self { id, values, metadata, epoch }
    }
    ...
    pub fn to_dict(&self) -> HashMap<String, Value> {
        ...
        dict.insert("metadata".to_string(), serde_json::to_value(&self.metadata).unwrap());
        dict.insert("epoch".to_string(), self.epoch.map(Value::from).unwrap_or(Value::Null));
        dict
    }
    ...
    pub fn from_dict(payload: &HashMap<String, Value>) -> Result<Self> {
        ...
        Ok(Self { id, values, metadata, epoch })
    }
}
...
impl CollectionState {
    ...
    pub fn to_dict(&self) -> HashMap<String, Value> {
        let mut dict = HashMap::new();
        dict.insert("vectors".to_string(), serde_json::to_value(&self.vectors).unwrap());
        dict.insert("indexes".to_string(), serde_json::to_value(&self.indexes).unwrap());
        dict
    }
    
    pub fn from_dict(payload: &HashMap<String, Value>) -> Self {
        let vectors = payload.get("vectors")
            .and_then(|v| serde_json::from_value(v.clone()).ok())
            .unwrap_or_else(HashMap::new);
        
        let indexes = payload.get("indexes")
            .and_then(|v| serde_json::from_value(v.clone()).ok())
            .unwrap_or_else(HashMap::new);
...
impl IndexManager {
    ...
    pub fn upsert_vectors(
        ...
        if records.is_empty() {
            return Ok(self.collections.get(collection).cloned().unwrap_or_default());
        }
        
        // Build updates first
        let mut updates = Vec::new();
        for record in records {
            let record_epoch = record.epoch.or(epoch)
                .ok_or_else(|| anyhow::anyhow!("Epoch must be provided either per record or as an argument"))?;
            
            let deterministic_updated_at = self.collections.get(collection)
                .and_then(|s| s.vectors.get(&record.id))
                .and_then(|v| v.get("updated_at").and_then(|v| v.as_str()).map(String::from))
            ...
            vector_payload.insert("metadata".to_string(), serde_json::to_value(&metadata).unwrap());
            vector_payload.insert("epoch".to_string(), Value::from(record_epoch));
            vector_payload.insert("updated_at".to_string(), Value::from(deterministic_updated_at));
            
            updates.push((record.id.clone(), vector_payload));
        }
        
        // Now apply updates
        let state = self.collections.entry(collection.to_string())
        ...
        let provider_name = state.indexes.get("provider")
            .and_then(|v| v.as_str())
            .unwrap_or("hnsw")
            .to_string();
        
        state.indexes.entry("provider".to_string())
            .or_insert_with(|| Value::from(provider_name.clone()));
        
        let proof_version = state.indexes.get("proof_version")
            .and_then(|v| v.as_i64())
            .unwrap_or(0) + 1;
        state.indexes.insert("proof_version".to_string(), Value::from(proof_version));
    ...
    pub fn delete_vectors(
        ...
        if removed {
            if let Some(ep) = epoch {
                let deletion_epochs = state.indexes.entry("deletion_epochs".to_string())
    ...
    pub fn get_collection_state(&mut self, collection: &str) -> CollectionState {
        self.collections.entry(collection.to_string())
            .or_insert_with(CollectionState::default)
            .clone()
    }
    ...
    pub fn search_vectors(
        ...
        if state.is_none() || state.unwrap().vectors.is_empty() {
            debug!("search requested for empty collection; collection={} has_state={}",
                collection, state.is_some());
            return Ok(Vec::new());
        }
        ...
        let plan = if use_ephemeral {
            ...
            self.provider_instances.get(collection)
                .and_then(|p| p.get_last_plan())
                .unwrap_or_else(HashMap::new)
        };
        self.last_search_plan = plan;
        
        let ranked: Vec<HashMap<String, Value>> = results.iter()
            .map(|(vector_id, score)| {
                ...
                result.insert("epoch".to_string(), payload.get("epoch").cloned().unwrap_or(Value::Null));
                result.insert("metadata".to_string(), payload.get("metadata").cloned().unwrap_or(Value::Null));
                result
            })
            .collect();
    ...
    pub fn build_index(&mut self, collection: &str) -> Result<HashMap<String, Value>> {
        ...
        params.insert("metric".to_string(), 
            state.indexes.get("metric").cloned().unwrap_or_else(|| Value::from("cosine")));
        ...
        status.insert("points_indexed".to_string(), Value::from(state.vectors.len()));
        ...
        status.insert("updated_at".to_string(), Value::from(Utc::now().to_rfc3339()));
        status.insert("proof_version".to_string(), 
            state.indexes.get("proof_version").cloned().unwrap_or(Value::from(0)));
        
        self.index_status.insert(collection.to_string(), status.clone());
        Ok(status)
    }
    ...
    pub fn get_index_status(&self, collection: &str) -> HashMap<String, Value> {
        ...
        status.insert("points_indexed".to_string(), Value::from(points_indexed));
        status.insert("provider".to_string(), 
            provider_name.map(|s| Value::from(s.clone())).unwrap_or(Value::Null));
    ...
    pub fn list_providers(&self) -> HashMap<String, HashMap<String, Value>> {
        ...
        for (name, (_, config)) in providers {
            ...
            info.insert("default_config".to_string(), serde_json::to_value(&config).unwrap());
    ...
    pub fn set_collection_provider(&mut self, collection: &str, provider_name: &str) -> Result<HashMap<String, Value>> {
        let providers = get_providers();
        ...
        let previous_provider = state.indexes.get("provider")
            .and_then(|v| v.as_str())
            .map(String::from);
        ...
        state.indexes.insert("provider".to_string(), Value::from(provider_name));
        state.indexes.insert("proof_version".to_string(), Value::from(proof_version));
        
        self.collections.insert(collection.to_string(), state.clone());
        self.collection_providers.insert(collection.to_string(), provider_name.to_string());
        self.provider_instances.remove(collection);
        
        let mut status = HashMap::new();
        status.insert("collection".to_string(), Value::from(collection));
        status.insert("provider".to_string(), Value::from(provider_name));
        status.insert("previous_provider".to_string(), 
        ...
        for entry in entries {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match fs::read_to_string(&path) {
                    Ok(content) => {
                        if let Ok(payload) = serde_json::from_str::<HashMap<String, Value>>(&content) {
                            ...
                            for (vector_id, vector_payload) in state.vectors.iter_mut() {
                                if let Some(metadata_val) = vector_payload.get("metadata") {
                                    if let Some(metadata_obj) = metadata_val.as_object() {
                                        let metadata: HashMap<String, Value> = metadata_obj.iter()
                                        ...
                                        vector_payload.insert("metadata".to_string(), 
                            ...
                            if let Some(provider_val) = state.indexes.get("provider") {
                                if let Some(provider_name) = provider_val.as_str() {
                                    self.collection_providers.insert(collection, provider_name.to_string());
                                }
                            }
                        }
                    }
    ...
    fn ensure_provider(&mut self, collection: &str) -> Result<&mut Box<dyn IndexProvider>> {
        ...
        if !self.provider_instances.contains_key(collection) {
            ...
            let state = self.collections.entry(collection.to_string())
                .or_insert_with(CollectionState::default);
            provider.build(&state.vectors);
            self.provider_instances.insert(collection.to_string(), provider);
            self.collection_providers.insert(collection.to_string(), provider_name);
        }
        
        Ok(self.provider_instances.get_mut(collection).unwrap())
    }
    
    fn get_ephemeral_provider(&mut self, collection: &str, provider_name: &str) -> Result<Box<dyn IndexProvider>> {
        let providers = get_providers();
        if !providers.contains_key(provider_name) {
            return Err(anyhow::anyhow!("unknown provider: {}", provider_name));
        }
    ...
    fn canonicalize_metadata(metadata: &HashMap<String, Value>, context: &str) -> HashMap<String, Value> {
        let mut removed = Vec::new();
        let canonical = Self::strip_volatile(metadata, "", &mut removed);
        
        if !removed.is_empty() {
            debug!("canonicalized metadata for {}; stripped volatile keys={:?}", 
                context, removed);
        }
    ...
    fn strip_volatile(value: &HashMap<String, Value>, prefix: &str, removed: &mut Vec<String>) -> HashMap<String, Value> {
        ...
        for (key, val) in value {
            ...
            if VOLATILE_KEY_NAMES.contains(&lower.as_str()) 
                || VOLATILE_KEY_SUFFIXES.iter().any(|suffix| lower.ends_with(suffix)) {
                removed.push(full_key);
                continue;
            }
            
            if let Some(obj) = val.as_object() {
                let nested: HashMap<String, Value> = obj.iter()
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect();
                canonical.insert(key.clone(), 
                    serde_json::to_value(&Self::strip_volatile(&nested, &full_key, removed)).unwrap());
...
mod tests {
    ...
    fn test_upsert_vectors() {
        ...
        let state = manager.upsert_vectors("test_collection", vec![record], None, None).unwrap();
    ...
    fn test_delete_vectors() {
        ...
        manager.upsert_vectors("test_collection", vec![record], None, None).unwrap();
        let state = manager.delete_vectors("test_collection", &["vec1".to_string()], Some(2)).unwrap();
