...
impl HNSWProvider {
    pub fn new(seed: Option<i64>, m: i32, ef_construction: i32, ef_search: i32, metric: String) -> Self {
...
impl IndexProvider for HNSWProvider {
    ...
    fn build(&mut self, records: &HashMap<String, HashMap<String, Value>>) {
        self.raw_vectors = records.iter()
            .map(|(id, payload)| {
                let vec = payload.get("vector")
    ...
    fn search(
        ...
        let effective_ef = extra_params.get("ef_search")
            .and_then(|v| v.as_i64())
        ...
        let query_norm = query_array.iter().map(|&x| x * x).sum::<f32>().sqrt().max(1.0);
        ...
        let coarse_scores: Vec<f32> = if self.metric == "cosine" {
            ...
            (0..total_vectors)
                .map(|i| {
                    ...
                    row.iter().zip(&normalized_query).map(|(&a, &b)| a * b).sum()
                })
                .collect()
        } else {
            (0..total_vectors)
        ...
        let mut indexed_scores: Vec<(usize, f32)> = coarse_scores.iter()
            .enumerate()
            .map(|(i, &score)| (i, score))
            .collect();
        ...
        let candidate_indices: Vec<usize> = indexed_scores.iter()
        ...
        let refined_scores: Vec<(usize, f32)> = candidate_indices.iter()
            .map(|&idx| {
        ...
        let counters = {
            ...
            map.insert("visited".to_string(), Value::from(candidate_count.min(total_vectors)));
            map.insert("scanned".to_string(), Value::from(candidate_count.min(total_vectors)));
            map.insert("candidate_count".to_string(), Value::from(candidate_count.min(total_vectors)));
            map.insert("total_points".to_string(), Value::from(total_vectors));
            map
        };
        
        let plan_name = if counters.get("visited").unwrap().as_u64().unwrap() >= total_vectors as u64
        ...
        params.insert("efConstruction".to_string(), Value::from(self.ef_construction));
        ...
        plan.insert("counters".to_string(), serde_json::to_value(&counters).unwrap());
        ...
        plan.insert("timings_ms".to_string(), serde_json::to_value(&timings).unwrap());
...
impl IVFPQProvider {
    ...
    fn initialize_centroids(&mut self, records: &HashMap<String, HashMap<String, Value>>) {
        ...
        for bucket in 0..bucket_count {
            if let Some((_, payload)) = record_items.get(bucket as usize) {
                if let Some(vec) = payload.get("vector")
                    .and_then(|v| v.as_array())
    ...
    fn assign_to_centroids(&self, query: &[f64]) -> Vec<i64> {
        let mut distances: Vec<(i64, f64)> = self.centroids.iter()
...
impl IndexProvider for IVFPQProvider {
    ...
    fn search(
        ...
        for centroid_id in assignments.iter().take(self.probes) {
            let mut bucket_records: Vec<String> = records.keys()
        ...
        let mut scored: Vec<(String, f64)> = candidate_ids.iter()
        ...
        counters.insert("candidate_count".to_string(), Value::from(candidate_ids.len()));
    ...
    fn snapshot(&self) -> HashMap<String, Value> {
        ...
        let centroids_map: serde_json::Map<String, Value> = self.centroids.iter()
    ...
    fn restore(&mut self, payload: &HashMap<String, Value>) {
        if let Some(centroids_val) = payload.get("centroids") {
            if let Some(centroids_obj) = centroids_val.as_object() {
                self.centroids = centroids_obj.iter()
                    .filter_map(|(k, v)| {
...
fn default_hnsw_config() -> HashMap<String, Value> {
    let mut config = HashMap::new();
    config.insert("m".to_string(), Value::from(
        env::var("HNSW_M").ok().and_then(|v| v.parse::<i64>().ok()).unwrap_or(16)
    ));
    config.insert("ef_construction".to_string(), Value::from(
        env::var("HNSW_EF_CONSTRUCTION").ok().and_then(|v| v.parse::<i64>().ok()).unwrap_or(200)
    ));
    config.insert("ef_search".to_string(), Value::from(
        env::var("HNSW_EF_SEARCH").ok().and_then(|v| v.parse::<i64>().ok()).unwrap_or(64)
    ));
    config.insert("metric".to_string(), Value::from(
        env::var("HNSW_METRIC").ok().unwrap_or_else(|| "cosine".to_string())
    ));
    config
}
...
pub fn get_providers() -> ProviderRegistry {
    ...
    fn hnsw_factory() -> Box<dyn IndexProvider> {
        ...
        let ef_construction = config.get("ef_construction").and_then(|v| v.as_i64()).unwrap_or(200) as i32;
        let ef_search = config.get("ef_search").and_then(|v| v.as_i64()).unwrap_or(64) as i32;
        let metric = config.get("metric").and_then(|v| v.as_str()).unwrap_or("cosine").to_string();
        Box::new(HNSWProvider::new(None, m, ef_construction, ef_search, metric))
    }
    
    providers.insert("hnsw".to_string(), (hnsw_factory as ProviderFactory, hnsw_config));
    ...
    providers.insert("ivf_pq".to_string(), (ivf_factory as ProviderFactory, ivf_config));
