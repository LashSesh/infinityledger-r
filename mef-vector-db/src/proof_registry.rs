...
fn canonicalize_metadata(metadata: Option<&Value>) -> Value {
    fn strip(value: &Value, prefix: &str) -> Option<Value> {
        match value {
            ...
            Value::Array(arr) => {
                let mut canonical: Vec<Value> = arr
                    .iter()
                    .filter_map(|item| strip(item, prefix))
                    .collect();
...
impl ProofRegistry {
    ...
    pub fn refresh_from_collections(&self, collections: &HashMap<String, CollectionState>) {
        ...
        state.collection_roots.retain(|k, _| active_collections.contains(k));
        state.collection_versions.retain(|k, _| active_collections.contains(k));
        state.proofs.retain(|(coll, _), _| active_collections.contains(coll));
        
        // Update or add collections
        for (collection, coll_state) in collections {
            let version = coll_state.indexes
                .get("proof_version")
                .and_then(|v| v.as_i64())
                .unwrap_or(0);
                ...
                || !state.collection_roots.contains_key(collection)
            {
                ...
                state.collection_versions.insert(collection.clone(), version);
                changed = true;
            }
        }
    ...
    pub fn get_membership_proof(&self, collection: &str, vector_id: &str) -> Option<MembershipProof> {
        let state = self.lock.lock().unwrap();
        state.proofs.get(&(collection.to_string(), vector_id.to_string())).cloned()
    }
    ...
    pub fn rotate_secret(&mut self, kid: Option<String>, secret: Option<String>) -> HashMap<String, String> {
