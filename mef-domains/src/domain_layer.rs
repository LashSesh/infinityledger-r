...
use mef_core::{MandorlaField, MEFCore};
...
impl DomainLayer {
    ...
    pub fn process_domain_data(
        ...
        let adapter = self.adapters.get(domain)
    ...
    fn homeomorphic_transfer(
        ...
        if !self.adapters.contains_key(target_domain) {
            return Err(anyhow!("Target domain adapter not found: {}", target_domain));
        }
...
mod tests {
    use super::*;
    use crate::adapter::{TextDomainAdapter, SignalDomainAdapter};
    use mef_core::MEFCoreConfig;

    fn create_test_domain_layer() -> DomainLayer {
        let mef_pipeline = Arc::new(MEFCore::new("test-seed", Some(MEFCoreConfig::default())).unwrap());
        let metatron_router = Arc::new(Mutex::new(MetatronRouter::new("/tmp/test_router")));
        
        let mut layer = DomainLayer::new(
            mef_pipeline,
            metatron_router,
            "/tmp/test_domains",
        ).unwrap();
